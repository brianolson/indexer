...
func (h *ImportHelper) Import(db idb.IndexerDb, args []string) {
	...
	ustat := updateAccounting(db, h.GenesisJsonPath, h.NumRoundsLimit)
	accountingRounds := ustat.Rounds
	txnCount := ustat.TxnCount
...
func UpdateAccounting(db idb.IndexerDb, genesisJsonPath string) (upstat UpdateStatus) {
	return updateAccounting(db, genesisJsonPath, 0)
}

func updateAccounting(db idb.IndexerDb, genesisJsonPath string, numRoundsLimit int) (stat UpdateStatus) {
	stat.Rounds = 0
	stat.TxnCount = 0
